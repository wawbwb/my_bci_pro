...
def calculate_model():
    ...
    events_marker = np.delete(events_marker, np.where(
        (events_marker[:, 2] != label_left_int) & (events_marker[:, 2] != label_right_int))[0], 0)
    ...
    for band_key, trials in trials_all.items():
        ...
        trials_test = {}
        for cl in cl_lab:
            trials_test[cl] = trials[cl][:, :,
                              trial_num_train[cl]:]  # note here is :  trial_num_train[cl]: means the test trials
